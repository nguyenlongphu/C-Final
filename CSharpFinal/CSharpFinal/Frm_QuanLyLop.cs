...
namespace CSharpFinal
{
    public partial class Frm_QuanLyLop : Form
    {
        ...
        {
            dataGridViewTimkiem.DataSource = lstlop;
        }

        private void txtTimKiemLop_TextChanged(object sender, EventArgs e)
        {
            string value = txtTimKiemLop.Text;
            if (!string.IsNullOrWhiteSpace(value))
            {
                dataGridViewTimkiem.DataSource = null;
                dataGridViewTimkiem.DataSource = lstlop;
                this.dataGridViewTimkiem.Columns[1].AutoSizeMode = DataGridViewAutoSizeColumnMode.Fill;
                List<Lop> temp = new List<Lop>();
                for (int i = 0; i < dataGridViewTimkiem.Rows.Count; i++)
                {
                    if (dataGridViewTimkiem.Rows[i].Cells[1].Value.ToString().ToLower().Contains(value.ToLower()))
                    {
                        temp.Add(lstlop[i]);
                    }
                }
                ...
                dataGridViewTimkiem.DataSource = null;
                dataGridViewTimkiem.DataSource = temp;
                this.dataGridViewTimkiem.Columns[1].AutoSizeMode = DataGridViewAutoSizeColumnMode.Fill;
